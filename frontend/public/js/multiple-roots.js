...
document.addEventListener("DOMContentLoaded", function() {
    ...
    console.log("Multiple Roots page loaded");

    const form = document.getElementById('multiple-roots-form');
    ...
    function displayResult(result) {
        ...
        if (result.iterations && result.xvalues && result.functionValues && result.firstDerivatives && result.secondDerivatives && result.errors) {
            for (let i = 0; i < result.iterations.length; i++) {
                ...
                row.innerHTML = `
                    ...
                    <td>${result.firstDerivatives[i]}</td>
                    <td>${result.secondDerivatives[i]}</td>
